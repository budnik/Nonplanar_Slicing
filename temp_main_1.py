...
stl_pfad_neu = stl_pfad.rsplit('.')[0] + '.gcode'
...
filtered_surface, limits = surface.create_surface(triangle_array, max_angle) # Winkel
# Calculate the nearest extrapolated points outside of the surface
xmesh, ymesh, zmesh = surface.create_surface_extended(filtered_surface, limits, printSetting.resolution)
# Calculate the gradient of the surface for extruding optimizing
gradx_mesh, grady_mesh, gradz = surface.create_gradient(filtered_surface, limits)
