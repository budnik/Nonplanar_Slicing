...
class slicer_config:    
    ...
    def __str__(self):
        if self.type == self.config_type.GCODE:
            return '; prusaslicer_config = begin\n; ' + '\n; '.join(f"{option_name} = {option}" for option_name, option in zip(*[self.options_names,self.options_set])) + '\n; prusaslicer_config = end'
        else: 
            return '# generated by PrusaSlicer 2.6.1+win64 on 2023-10-27 at 08:10:10 UTC' + ''.join(f"{option_name} = {option}\n" for option_name, option in zip(*[self.options_names,self.options_set]))
